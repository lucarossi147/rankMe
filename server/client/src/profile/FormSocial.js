...
function FormSocial(props){
    ...
    const handleSubmit = (evt) => {
        ...
        if(!facebook.startsWith('www.facebook.com')){
            errorNotify("Not a facebook link :(")
            return
        }

        if(!instagram.startsWith('www.instagram.com')){
            errorNotify("Not an instagram link :(")
            return
        }
...
const InstagramButton = (props) => {
    ...
    return (
        <button onClick={() => openLink(link)} type="button" className="btn btn-outline-danger social">
